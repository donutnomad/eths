...
func methodIDBytes(id []byte) string {
	if len(id) < 4 {
		return "[4]byte{}"
	}
	return fmt.Sprintf("[4]byte{0x%02x, 0x%02x, 0x%02x, 0x%02x}", id[0], id[1], id[2], id[3])
}

func hashIDBytes(id common.Hash) string {
	return methodIDBytes(id.Bytes()[:4])
}

func useInputStruct(method *tmplMethod) bool {
	return len(method.Normalized.Inputs) > 4
}

func methodIsConstant(method *tmplMethod) bool {
	return method.Original.IsConstant()
}

func inputFieldName(name string) string {
	field := abi.ToCamelCase(name)
	switch field {
	case "Name", "MethodID":
		return "Input" + field
	default:
...
func joinedTypeName(parts ...string) string {
	var nonEmpty []string
...
func methodTypeName(contract, name string) string {
	return joinedTypeName(contract, name, "Method")
}

func inputTypeName(contract, name string) string {
	return joinedTypeName(contract, name, "Input")
}

func outputTypeName(contract, name string) string {
	return joinedTypeName(contract, name, "Output")
}

func eventTypeName(contract, name string) string {
	return joinedTypeName(contract, name, "Event")
}

func errorTypeName(contract, name string) string {
	return joinedTypeName(contract, name, "Error")
}
...
func BindV2(types []string, abis []string, bytecodes []string, pkg string, libs map[string]string, aliases map[string]string) (string, error) {
	...
	funcs := map[string]interface{}{
		...
		"methodidbytes":      methodIDBytes,
		"hashidbytes":        hashIDBytes,
		"useinputstruct":     useInputStruct,
		"methodisconstant":   methodIsConstant,
		"inputfieldname":     inputFieldName,
		"joinedtypename":     joinedTypeName,
