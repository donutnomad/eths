...
func (cfg *Config) process() (code []byte, err error) {
	...
	if cfg.FieldOverride != "" {
		overrideNames := strings.Split(cfg.FieldOverride, ",")
		for _, name := range overrideNames {
			name = strings.TrimSpace(name)
			if name == "" {
				continue
			}
			otyp, err := lookupStructType(pkg.Scope(), name)
			if err != nil {
				return nil, fmt.Errorf("can't find field replacement type %s: %v", name, err)
			}
			if err := mtyp.loadOverrides(otyp); err != nil {
				return nil, err
			}
		}
	}
...
func generate(mtyp *marshalerType, cfg *Config) ([]byte, error) {
	...
	fmt.Fprintln(w)
	if len(mtyp.overrides) > 0 {
		for _, ov := range mtyp.overrides {
			writeUseOfOverride(w, ov, mtyp.scope.qualify)
		}
	}
...
type marshalerField struct {
	...
	embedPrefix string      // non-empty for fields from embedded structs (e.g. "Tx" for t.Tx.Field)
}
...
func newMarshalerType(fs *token.FileSet, imp types.Importer, typ *types.Named) *marshalerType {
	...
	// Collect direct (non-embedded) field names first for precedence.
	directFields := make(map[string]bool)
	for i := 0; i < styp.NumFields(); i++ {
		f := styp.Field(i)
		if f.Exported() && !f.Anonymous() {
			directFields[f.Name()] = true
		}
	}

	// Track embedded field names to detect ambiguity across multiple embedded structs.
	// Maps field name to the number of embedded structs that define it.
	embFieldCount := make(map[string]int)
	type embField struct {
		field   *types.Var
		tag     string
		embName string // name of the embedding field (e.g. "Tx", "Receipt")
	}
	var embFields []embField

	// First pass: collect all embedded fields and count occurrences.
	for i := 0; i < styp.NumFields(); i++ {
		f := styp.Field(i)
		if !f.Exported() || !f.Anonymous() {
			continue
		}
		embType := types.Unalias(f.Type())
		if ptr, ok := embType.(*types.Pointer); ok {
			embType = types.Unalias(ptr.Elem())
		}
		named, ok := embType.(*types.Named)
		if !ok {
			fmt.Fprintf(os.Stderr, "Warning: ignoring embedded field %s (not a named type)\n", f.Name())
			continue
		}
		embStruct, ok := named.Underlying().(*types.Struct)
		if !ok {
			fmt.Fprintf(os.Stderr, "Warning: ignoring embedded field %s (not a struct)\n", f.Name())
			continue
		}
		mtyp.scope.addReferences(embStruct)
		for j := 0; j < embStruct.NumFields(); j++ {
			ef := embStruct.Field(j)
			if !ef.Exported() {
				continue
			}
			if ef.Anonymous() {
				fmt.Fprintf(os.Stderr, "Warning: ignoring nested embedded field %s\n", ef.Name())
				continue
			}
			if directFields[ef.Name()] {
				continue // direct fields take precedence
			}
			embFieldCount[ef.Name()]++
			embFields = append(embFields, embField{field: ef, tag: embStruct.Tag(j), embName: f.Name()})
		}
	}
	...
	// Add embedded fields. For ambiguous fields (same name in multiple embedded structs),
	// use the first occurrence and set embedPrefix so generated code uses qualified access.
	seen := make(map[string]bool)
	for _, ef := range embFields {
		name := ef.field.Name()
		if seen[name] {
			continue // already added from an earlier embedded struct
		}
		seen[name] = true
		mf := &marshalerField{
			name:    name,
			typ:     ef.field.Type(),
			origTyp: ef.field.Type(),
			tag:     ef.tag,
		}
		// All embedded fields need a prefix for qualified access since
		// ambiguous fields can't use simple promotion.
		if embFieldCount[name] > 1 {
			mf.embedPrefix = ef.embName
		}
		mtyp.Fields = append(mtyp.Fields, mf)
	}
...
func (mtyp *marshalerType) loadOverrides(otyp *types.Named) error {
	...
	for i := 0; i < s.NumFields(); i++ {
		of := s.Field(i)
		if !of.Exported() {
			return fmt.Errorf("%v: field override type cannot have unexported fields", mtyp.fs.Position(of.Pos()))
		}
		if of.Anonymous() {
			// Resolve the underlying struct of the embedded override type.
			embType := types.Unalias(of.Type())
			if ptr, ok := embType.(*types.Pointer); ok {
				embType = types.Unalias(ptr.Elem())
			}
			named, ok := embType.(*types.Named)
			if !ok {
				return fmt.Errorf("%v: embedded override field %s is not a named type", mtyp.fs.Position(of.Pos()), of.Name())
			}
			embStruct, ok := named.Underlying().(*types.Struct)
			if !ok {
				return fmt.Errorf("%v: embedded override field %s is not a struct", mtyp.fs.Position(of.Pos()), of.Name())
			}
			for j := 0; j < embStruct.NumFields(); j++ {
				eof := embStruct.Field(j)
				if !eof.Exported() || eof.Anonymous() {
					continue
				}
				f := mtyp.fieldByName(eof.Name())
				if f == nil {
					if fun, retType := findFunction(mtyp.orig, eof.Name(), eof.Type()); fun != nil {
						f = &marshalerField{name: fun.Name(), origTyp: retType, typ: eof.Type(), function: fun, tag: embStruct.Tag(j)}
						mtyp.Fields = append(mtyp.Fields, f)
					} else {
						return fmt.Errorf("%v: no matching field or function for %s in original type %s", mtyp.fs.Position(eof.Pos()), eof.Name(), mtyp.name)
					}
				}
				if err := checkConvertible(eof.Type(), f.origTyp); err != nil {
					return fmt.Errorf("%v: invalid field override: %v", mtyp.fs.Position(eof.Pos()), err)
				}
				f.typ = eof.Type()
			}
			mtyp.scope.addReferences(embStruct)
	...
	mtyp.overrides = append(mtyp.overrides, otyp)
	return nil
}
