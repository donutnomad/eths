...
func (m *marshalMethod) marshalConversions(from, to Var, fieldName string) (s []Statement) {
	for _, f := range m.mtyp.Fields {
		accessFrom := qualifiedAccess(from, f)
...
// qualifiedAccess returns the accessor expression for a field.
// For fields with embedPrefix (ambiguous embedded fields), it generates
// receiver.EmbeddedType.FieldName instead of receiver.FieldName.
func qualifiedAccess(receiver Expression, f *marshalerField) Dotted {
	if f.embedPrefix != "" {
		return Dotted{Receiver: Dotted{Receiver: receiver, Name: f.embedPrefix}, Name: f.name}
	}
	return Dotted{Receiver: receiver, Name: f.name}
}
